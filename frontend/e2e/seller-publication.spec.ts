...
async function mockSellerApi(page: Page) {
  ...
  await page.route('https://pay-sandbox.sepay.vn/**', async (route) => {
    await route.fulfill({
      status: 302,
      headers: {
        location: `http://127.0.0.1:4173/seller/upgrade?payment=success&orderCode=${paymentOrderCode}`,
      },
    })
  })
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path.endsWith('/auth/refresh'))
      data = {
        accessToken: 'access',
        tokenType: 'Bearer',
        user: { ...user, role: paid ? 'SELLER' : 'USER' },
      }
    else if (path.endsWith('/users/me'))
      data = {
        ...user,
        role: paid ? 'SELLER' : 'USER',
        phone: '',
        status: 'ACTIVE',
        createdAt: '2026-08-01T00:00:00',
      }
    else if (path.endsWith('/payments/sepay/seller-upgrade/offer'))
      data = {
        amount: 99000,
        currency: 'VND',
        provider: 'SEPAY',
    ...
    else if (path.endsWith('/payments/sepay/seller-upgrade') && method === 'POST')
      data = {
        payment: payment(false),
        checkoutUrl: 'https://pay-sandbox.sepay.vn/v1/checkout/init',
        method: 'POST',
        fields: {
          merchant: 'SP-TEST-E2E',
          operation: 'PURCHASE',
          payment_method: 'BANK_TRANSFER',
          order_invoice_number: paymentOrderCode,
          order_amount: '99000',
          currency: 'VND',
          signature: 'e2e-signature',
        },
      }
    else if (path.endsWith(`/payments/sepay/seller-upgrade/${paymentOrderCode}`)) {
      paid = true
      data = payment(true)
      ...
      status = 'DRAFT'
      data = { ...baseListing, ...request.postDataJSON(), status }
    ...
    else if (path.endsWith('/seller/listings/501/submit')) {
      status = 'PENDING'
      data = { ...baseListing, status }
...
function payment(success: boolean) {
  return {
    ...
    providerOrderId: success ? 'sepay-order-e2e' : null,
    providerTransactionId: success ? 'sepay-transaction-e2e' : null,
    failureReason: null,
    expiresAt: '2026-08-17T12:15:00',
    completedAt: success ? '2026-08-17T12:01:00' : null,
    createdAt: '2026-08-17T12:00:00',
    updatedAt: '2026-08-17T12:01:00',
  }
}

test('USER pays through SePay, creates DRAFT, uploads two images and submits PENDING', async ({
  page,
}) => {
