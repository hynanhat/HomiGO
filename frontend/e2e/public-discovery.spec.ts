...
function pageOf(content: unknown[]) { return { content, number: 0, size: 12, totalElements: content.length, totalPages: content.length ? 1 : 0, numberOfElements: content.length, first: true, last: true, empty: !content.length } }
async function mockPublicApi(page: Page) {
  await page.route('**/api/v1/**', async (route) => {
    ...
    else if (path.includes('/wards')) data = pageOf([{ id: 41, districtId: 32, name: 'Phường Thảo Điền', code: 'THAO-DIEN' }])
...
test('guest searches, filters and opens a listing by publicCode', async ({ page }) => {
  ...
  if (page.viewportSize()!.width < 1024) {
    ...
    await page.getByRole('dialog', { name: 'Bộ lọc tìm kiếm' }).getByLabel('Tỉnh / thành phố').selectOption('21')
    await page.getByRole('button', { name: 'Xem kết quả' }).click()
  } else {
    await page.getByLabel('Tỉnh / thành phố').selectOption('21')
  }
  await page.getByRole('link', { name: /Xem Căn hộ hai phòng ngủ/ }).first().click()
