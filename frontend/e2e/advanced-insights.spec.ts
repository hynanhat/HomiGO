...
const recommended = {
  ...listing,
  id: 102,
  publicCode: 'HMG-2026-000102',
  userId: 3,
  title: 'Căn hộ ven sông cùng khu vực',
  price: 6_100_000_000,
}
const notification = {
  id: 701,
...
async function mockAdvancedApi(page: Page, authenticated = false) {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path.endsWith('/auth/refresh') && authenticated)
      ...
      data = { count: notificationRead ? 0 : 1 }
    else if (path.endsWith('/notifications/read-all')) data = { updatedCount: 1 }
    else if (/\/notifications\/\d+\/read$/.test(path)) {
      notificationRead = true
      data = { ...notification, read: true, readAt: '2026-08-17T11:00:00' }
    } else if (path.endsWith('/notifications'))
      data = pageOf([
        {
          ...notification,
          read: notificationRead,
          readAt: notificationRead ? '2026-08-17T11:00:00' : null,
        },
      ])
    else if (path.endsWith('/listings/HMG-2026-000101/views') && method === 'POST') {
      viewRequests += 1
      ...
      data = {
        ...
        last7DaysViews: 46,
        periodDays: 30,
        dailyViews: Array.from({ length: 30 }, (_, index) => ({
