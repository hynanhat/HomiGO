...
const listing = { id: 101, publicCode: 'HMG-2026-000101', userId: 2, version: 1, title: 'Căn hộ hai phòng ngủ tại Thảo Điền', description: 'Không gian sáng.', categoryName: 'Căn hộ', projectName: 'Homi Riverside', provinceName: 'TP. Hồ Chí Minh', districtName: 'Thành phố Thủ Đức', address: '12 Nguyễn Văn Hưởng', price: 5800000000, area: 82, bedrooms: 2, bathrooms: 2, contactName: 'Trần Bình', contactPhone: '0901234567', status: 'ACTIVE', images: [], createdAt: '2026-08-01T08:00:00', updatedAt: '2026-08-14T10:30:00' }
const project = { id: 201, name: 'Homi Riverside', slug: 'homi-riverside', investor: 'Homi Group', districtId: 32, districtName: 'Thành phố Thủ Đức', address: '12 Nguyễn Văn Hưởng', status: 'IN_PROGRESS', priceFrom: 4500000000, updatedAt: '2026-08-12T09:00:00' }
...
async function mockApi(page: Page) {
  ...
  await page.route('**/api/v1/**', async (route) => {
    const request = route.request(); const path = new URL(request.url()).pathname; let data: unknown = null
    if (path.endsWith('/auth/register')) data = user
    else if (path.endsWith('/auth/login')) { authenticated = true; data = { accessToken: 'access', tokenType: 'Bearer', user } }
    else if (path.endsWith('/auth/logout')) { authenticated = false; data = null }
    else if (path.endsWith('/auth/refresh') && authenticated) data = { accessToken: 'rotated', tokenType: 'Bearer', user }
    ...
    else if (path.endsWith('/users/me') && request.method() === 'PUT') { profile = { ...profile, ...request.postDataJSON() }; data = profile }
    else if (path.endsWith('/users/me')) data = profile
    else if (path.endsWith('/saved-listings') && request.method() === 'GET') data = pageOf([listing])
