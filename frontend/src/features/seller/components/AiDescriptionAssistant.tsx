...
export function AiDescriptionAssistant({
  ...
  const temporarilyReserved = Boolean(
    quota && quota.availableNow <= 0 && quota.remainingAttempts > 0,
  )
  ...
  const generate = async () => {
    ...
    if (
      preview &&
      !window.confirm('Tạo lại sẽ thay thế bản xem trước hiện tại. Bạn muốn tiếp tục?')
    )
      return
    setMessage(null)
    try {
      ...
      if (apiError.errorCode === 'AI_DAILY_LIMIT_REACHED')
        setMessage('Bạn đã dùng hết 5 lượt hôm nay. Hạn mức sẽ được đặt lại vào ngày mai.')
      else if (apiError.errorCode === 'AI_QUOTA_TEMPORARILY_RESERVED')
        setMessage('Các lượt đang được xử lý. Vui lòng chờ một lát rồi thử lại.')
      else
        setMessage(
          getSafeErrorMessage(error, 'Chưa thể tạo mô tả. Bạn vẫn có thể nhập mô tả thủ công.'),
        )
    }
  }
  ...
  const apply = () => {
    ...
    if (
      ...
      !window.confirm('Áp dụng sẽ thay thế mô tả bạn đang nhập. Bạn muốn tiếp tục?')
    )
      return
    onApply(preview)
    setPreview(null)
  ...
  return (
    <Card className="grid gap-4 border-violet-200 bg-violet-50/40 p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-600">
            AI kết hợp từ khóa với thông tin form. Hãy kiểm tra nội dung trước khi đăng.
          </p>
        </div>
        {quota && (
          <span
            className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-violet-800"
            aria-live="polite"
          >
      ...
      {quotaQuery.isError && (
        <p className="text-sm text-red-700" role="alert">
          Không thể kiểm tra hạn mức AI. Bạn vẫn có thể nhập mô tả thủ công.
        </p>
      )}
      {quota && !quota.enabled && (
        <p className="text-sm text-amber-800" role="status">
          Tính năng AI đang tạm ngưng. Bạn vẫn có thể nhập mô tả thủ công.
        </p>
      )}
      {exhausted && (
        <p className="text-sm text-amber-800" role="status">
          Đã hết 5 lượt hôm nay{resetTime ? `; đặt lại lúc ${resetTime}` : ''}.
        </p>
      )}
      {temporarilyReserved && (
        <p className="text-sm text-amber-800" role="status">
          Các lượt đang được xử lý
          {retryTime ? `; thử lại sau ${retryTime}` : '; vui lòng thử lại sau'}.
        </p>
      )}
      {message && (
        <p className="text-sm font-medium text-ink-800" role="status" aria-live="polite">
          {message}
        </p>
      )}
      ...
      <div>
        <Button
          ...
          disabled={disabled || quotaQuery.isLoading}
          onClick={generate}
        >
          <Sparkles className="size-4" aria-hidden="true" />
          {preview ? 'Tạo lại mô tả' : 'Tạo mô tả'}
        </Button>
      </div>
      ...
      {preview && (
        <div
          ref={previewRef}
          tabIndex={-1}
          className="grid gap-3 rounded-lg border border-violet-200 bg-white p-4 outline-none focus:ring-2 focus:ring-violet-500"
        >
          <div>
            <h4 className="font-bold text-ink-950">Bản xem trước</h4>
            {stalePreview && (
              <p className="mt-1 text-sm text-amber-800" role="alert">
                Thông tin form đã thay đổi sau khi tạo. Hãy tạo lại để mô tả chính xác hơn.
              </p>
            )}
          </div>
