    ...
    <section aria-labelledby="recommendation-title">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 id="recommendation-title" className="mt-2 text-2xl font-extrabold text-ink-950">
            Bất động sản dành cho bạn
          </h2>
          <p className="mt-1 text-sm text-ink-600">
            Được xếp hạng theo loại hình, vị trí, mức giá và diện tích tương đồng.
          </p>
        </div>
        <Link
      ...
      {query.isError && (
        <p className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          Gợi ý tạm thời chưa khả dụng. Bạn vẫn có thể tiếp tục xem thông tin tin đăng.
        </p>
      )}
      {query.data?.length === 0 && (
        <p className="mt-5 rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-ink-600">
          Chưa có bất động sản tương tự đang hoạt động.
        </p>
      )}
            ...
            <Card key={listing.id} className="recommendation-card overflow-hidden">
              <Link to={`/listings/${listing.publicCode}`} className="group block">
                ...
                <div className="p-4">
                  ...
                  <h3 className="mt-3 line-clamp-2 font-bold text-ink-950 group-hover:text-brand-700">
                    {listing.title}
                  </h3>
                  <p className="mt-2 text-lg font-extrabold text-brand-700">
                    {formatCurrency(listing.price)}
                  </p>
                  <p className="mt-2 flex items-start gap-1 text-sm text-ink-600">
                  ...
                  <ul className="mt-3 flex flex-wrap gap-1.5" aria-label="Lý do gợi ý">
                    {reasons.map((reason) => (
                      <li
                        key={reason}
                        className="rounded-md bg-slate-100 px-2 py-1 text-xs font-semibold text-ink-700"
                      >
                        {reason}
                      </li>
                    ))}
