...
export const projectKeys = {
  ...
  detail: (slug: string, page: number, size: number) =>
    [...projectKeys.all, 'detail', slug, page, size] as const,
}

export function useProjectSearch(state: ProjectSearchState) {
  return useQuery(
    queryOptions({ queryKey: projectKeys.search(state), queryFn: () => searchProjects(state) }),
  )
}
