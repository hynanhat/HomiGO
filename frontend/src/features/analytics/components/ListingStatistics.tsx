  ...
  if (query.isError || !query.data) {
    return (
      <ErrorState
        ...
        description="Số liệu lượt xem hiện chưa khả dụng."
    ...
    <Card className="p-6">
      ...
      <dl className="mt-6 grid gap-3 sm:grid-cols-3">
        {metrics.map(({ label, value, icon: Icon }) => (
          <div key={label} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            <dt className="flex items-center gap-2 text-sm font-medium text-ink-600">
              <Icon className="size-4 text-brand-700" aria-hidden="true" />
              {label}
            </dt>
            <dd className="mt-2 text-2xl font-extrabold text-ink-950">
      ...
      <div className="mt-7">
        ...
        <div
          ...
          role="img"
          aria-label={`Biểu đồ lượt xem trong ${days} ngày`}
        >
          {statistics.dailyViews.map((item) => (
            <div
              key={item.date}
              className="analytics-bar-column"
              title={`${formatDate(item.date)}: ${item.views} lượt xem`}
            >
              <span className="analytics-bar-value">{item.views}</span>
              <span
                className="analytics-bar"
                style={{ height: `${Math.max(item.views ? 8 : 2, (item.views / maximum) * 100)}%` }}
              />
        ...
        <details className="mt-4 text-sm text-ink-700">
          <summary className="cursor-pointer font-semibold text-brand-700">
            Xem số liệu dạng danh sách
          </summary>
          <ul className="mt-3 grid gap-1 sm:grid-cols-2 lg:grid-cols-3">
            {statistics.dailyViews.map((item) => (
              <li key={item.date}>
                {formatDate(item.date)}: {item.views} lượt xem
              </li>
            ))}
          </ul>
        </details>
