...
import { getNotifications, getUnreadCount, markAllNotificationsRead, markNotificationRead } from './notificationApi'
...
describe('notification API', () => {
  it('sends inbox filters and unwraps notification data', async () => {
    server.use(http.get('*/api/v1/notifications', ({ request }) => {
      ...
      return HttpResponse.json({ success: true, message: 'OK', errorCode: null, data: { content: [], number: 2, size: 5, totalElements: 0, totalPages: 0, numberOfElements: 0, first: false, last: true, empty: true } })
    }))
