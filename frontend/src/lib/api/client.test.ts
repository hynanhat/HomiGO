...
describe('API refresh coordination', () => {
  it('performs one refresh for concurrent 401 responses and retries with rotated token', async () => {
    ...
    server.use(
      http.get('*/api/v1/protected-test', ({ request }) =>
        request.headers.get('authorization') === 'Bearer rotated'
          ? HttpResponse.json({ success: true, data: 'ok', message: 'OK', errorCode: null })
          : HttpResponse.json(
              { success: false, data: null, message: 'Expired', errorCode: 'UNAUTHORIZED' },
              { status: 401 },
            ),
      ),
    )
    await expect(
      Promise.all([apiClient.get('/protected-test'), apiClient.get('/protected-test')]),
  ...
  it('terminates a revoked session without a request loop', async () => {
    ...
    server.use(
      http.get('*/api/v1/revoked-test', () =>
        HttpResponse.json(
          { success: false, data: null, message: 'Revoked', errorCode: 'UNAUTHORIZED' },
          { status: 401 },
        ),
      ),
    )
