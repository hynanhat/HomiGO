...
describe('SellerUpgradePage', () => {
  it('treats the browser success callback as pending until backend IPN status succeeds', async () => {
    localStorage.setItem('homigo.session.v1', JSON.stringify({
    ...
    render(
      <QueryClientProvider client={client}>
        <AuthProvider>
          <ToastProvider>
            <MemoryRouter initialEntries={['/seller/upgrade?payment=success&orderCode=HMG-SEPAY-FIXTURE001']}>
              <SellerUpgradePage />
            </MemoryRouter>
          </ToastProvider>
        </AuthProvider>
    ...
    expect(await screen.findByRole('heading', { name: 'Đang chờ SePay xác nhận' })).toBeInTheDocument()
