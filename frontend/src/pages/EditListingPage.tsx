...
export default function EditListingPage() {
  ...
  const update = useUpdateSellerListing(id)
  ...
  const initial: ListingFormValues = {
    ...
    legalStatus: item.legalStatus ?? undefined,
  ...
  const submit = async (values: ListingFormValues) => {
    try {
      ...
      showToast({
        ...
        description:
          item.status === 'ACTIVE' ? 'Tin đang hiển thị sẽ chuyển về chờ duyệt.' : undefined,
      })
      ...
      if (error instanceof ApiError && error.status === 409) {
        showToast({
          type: 'warning',
          title: 'Tin đã được thay đổi ở nơi khác',
          description: 'Hãy tải lại phiên bản mới trước khi tiếp tục.',
        })
        return
      }
      showToast({
    ...
    <Card className="p-6">
      <div className="flex justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-600">Phiên bản hiện tại: {item.version}</p>
        </div>
