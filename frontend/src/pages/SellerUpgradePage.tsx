    ...
    return (
      <section className="rounded-xl border border-emerald-200 bg-emerald-50 p-5" role="status">
        <div className="flex gap-3">
          <BadgeCheck className="size-6 shrink-0 text-emerald-700" aria-hidden="true" />
          <div>
            <h2 className="font-bold text-emerald-950">Thanh toán đã được xác nhận</h2>
            <p className="mt-1 text-sm text-emerald-800">
              Tài khoản của bạn đã có quyền người bán.
            </p>
          </div>
        </div>
      </section>
  ...
  if (callback === 'success' && payment?.status === 'PENDING') {
    return (
      <section className="rounded-xl border border-amber-200 bg-amber-50 p-5" role="status">
        <div className="flex gap-3">
          <Clock3 className="size-6 shrink-0 text-amber-700" aria-hidden="true" />
          <div>
            <h2 className="font-bold text-amber-950">Đang chờ SePay xác nhận</h2>
            <p className="mt-1 text-sm text-amber-900">
              Bạn đã quay lại từ cổng thanh toán. Trang đang tự kiểm tra IPN; chưa có quyền Seller
              cho đến khi backend xác nhận.
            </p>
          </div>
        </div>
      </section>
  ...
  if (callback === 'cancel' || callback === 'error') {
    return (
      <section className="rounded-xl border border-red-200 bg-red-50 p-5" role="alert">
        <div className="flex gap-3">
          <CircleAlert className="size-6 shrink-0 text-red-700" aria-hidden="true" />
          <div>
            <h2 className="font-bold text-red-950">Thanh toán chưa hoàn tất</h2>
            <p className="mt-1 text-sm text-red-800">
              SePay báo giao dịch bị hủy hoặc gặp lỗi. Tài khoản chưa được nâng cấp và bạn có thể
              thử lại.
            </p>
          </div>
        </div>
  ...
  if (payment?.status === 'EXPIRED') {
    return (
      <section className="rounded-xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-700">
        Đơn trước đã hết hạn. Hãy tạo checkout mới để tiếp tục.
      </section>
    )
  }
  return null
...
export default function SellerUpgradePage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        showToast({
          type: 'error',
          title: 'Thanh toán thành công nhưng chưa làm mới được phiên',
  ...
  const startCheckout = async () => {
    try {
      ...
      showToast({
        type: 'error',
        title: 'Không thể mở SePay Sandbox',
        description: getSafeErrorMessage(error),
      })
    }
  }
  ...
  return (
    <main className="px-4 py-10 sm:py-14 lg:py-16">
      <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1.15fr_.85fr]">
        <section className="space-y-6">
          <div>
            <p className="eyebrow">HomiGO Seller</p>
            <h1 className="mt-3 max-w-2xl text-3xl font-bold tracking-tight text-ink-950 sm:text-4xl">
              Mở quyền đăng tin chuyên nghiệp
            </h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-ink-600">
              Thanh toán một lần qua SePay Sandbox. Quyền Seller chỉ được cấp sau khi HomiGO nhận và
              xác minh IPN từ SePay.
            </p>
          </div>

          <CallbackStatus callback={callback} payment={payment.data} />
          {payment.isError && orderCode && (
            <ErrorState
              title="Không thể kiểm tra đơn thanh toán"
              description="Đơn không tồn tại hoặc không thuộc tài khoản hiện tại."
          ...
          <Card className="p-6 sm:p-8">
            <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-ink-600">
                  Thanh toán thử nghiệm, không phát sinh tiền thật.
                </p>
              </div>
              <div className="sm:text-right">
                <p className="text-sm font-semibold text-ink-600">Thanh toán một lần</p>
            ...
            {!offer.data.configured && (
              <div
                className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"
                role="alert"
              >
                Backend chưa có `SEPAY_MERCHANT_ID` và `SEPAY_SECRET_KEY`. Hãy cấu hình credential
                Sandbox mới trước khi thanh toán.
              </div>
            )}

            {alreadySeller ? (
              <Link
                className="mt-7 inline-flex min-h-12 items-center justify-center rounded-lg bg-brand-600 px-5 font-semibold text-white hover:bg-brand-700"
                to="/seller"
              >
              ...
              <Button
                ...
                size="lg"
                loading={createCheckout.isPending || syncingRole}
        ...
        <aside>
          <Card className="p-6">
            <div className="flex items-center gap-3">
              <History className="size-5 text-brand-700" aria-hidden="true" />
              <h2 className="text-lg font-bold">Lịch sử nâng cấp</h2>
            </div>
            {history.isPending && <Skeleton className="mt-5 h-40" />}
            {history.isError && (
              <div className="mt-5">
                <ErrorState
                  description="Không tải được lịch sử thanh toán."
                  onRetry={() => history.refetch()}
                />
              </div>
            )}
            {history.data?.empty && (
              <p className="mt-5 rounded-lg bg-slate-50 p-4 text-sm text-ink-600">
                Bạn chưa có giao dịch nâng cấp nào.
              </p>
            )}
                  ...
                  <article key={item.id} className="rounded-lg border border-slate-200 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate text-sm font-bold text-ink-950">{item.orderCode}</p>
                        <p className="mt-1 text-xs text-ink-600">{formatDate(item.createdAt)}</p>
                      </div>
