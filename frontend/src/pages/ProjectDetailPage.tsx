...
export default function ProjectDetailPage() {
  ...
  useEffect(() => {
    if (query.data) document.title = `${query.data.name} | HomiGO`
  }, [query.data])
  if (query.isPending)
    return (
    ...
    return (
      <main className="mx-auto max-w-4xl px-4 py-16">
        <ErrorState
          ...
          description="Đường dẫn dự án không tồn tại hoặc dữ liệu đang tạm ngừng."
    ...
    <main className="min-h-screen bg-slate-50">
      <section className="bg-ink-950 text-white">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 flex flex-wrap items-start justify-between gap-5">
            ...
            <Card className="min-w-64 p-5 text-ink-950">
              <p className="text-sm text-ink-600">Giá tham khảo</p>
              <p className="mt-1 text-xl font-extrabold text-brand-700">
                {project.priceFrom ? `Từ ${formatCurrency(project.priceFrom)}` : 'Liên hệ'}
              </p>
      ...
      <div className="mx-auto max-w-7xl space-y-12 px-4 py-10 sm:px-6 lg:px-8">
        <section>
          ...
          <p className="mt-4 max-w-4xl whitespace-pre-line leading-7 text-ink-700">
            {project.description}
          </p>
        </section>
        <section>
          <h2 className="text-2xl font-bold">Tin đăng đang hoạt động</h2>
          {project.listings.empty ? (
            <div className="mt-5">
              <EmptyState title="Dự án chưa có tin đăng" />
            </div>
          ) : (
          ...
          <div className="mt-7">
            <Pagination
              page={project.listings.number}
              totalPages={project.listings.totalPages}
