...
export default function UserManagementPage() {
  ...
  const ban = useAdminMutation(({ id, reason }: { id: number; reason: string }) =>
    banUser(id, reason),
  )
  const unban = useAdminMutation((id: number) => unbanUser(id))
  const runBan = async () => {
    if (!selected || !reason.trim()) return
    try {
      await ban.mutateAsync({ id: selected.id, reason })
      setSelected(undefined)
      setReason('')
      showToast({ type: 'success', title: 'Đã khóa tài khoản' })
    } catch (error) {
      showToast({ type: 'error', title: 'Không thể khóa', description: getSafeErrorMessage(error) })
    }
  }
  const columns = [
    {
    ...
    { key: 'role', header: 'Vai trò', render: (item: AdminUser) => item.role },
    {
      key: 'status',
      header: 'Trạng thái',
      render: (item: AdminUser) => (
  ...
  return (
    <div>
      <h2 className="text-3xl font-extrabold">Quản lý người dùng</h2>
