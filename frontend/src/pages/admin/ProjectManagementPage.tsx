...
export default function ProjectManagementPage() {
  ...
  const [editing, setEditing] = useState<ProjectSummary | null>()
  const [deleting, setDeleting] = useState<ProjectSummary>()
  const [form, setForm] = useState(empty)
  const { showToast } = useToast()
  const save = useAdminMutation((value: { id?: number; request: ProjectRequest }) =>
    value.id ? updateProject(value.id, value.request) : createProject(value.request),
  )
  const remove = useAdminMutation(deleteProject)
  const create = () => {
    setEditing(null)
    setForm(empty)
  }
  const edit = (item: ProjectSummary) => {
    setEditing(item)
    setForm({
      name: item.name,
      slug: item.slug,
  ...
  const submit = async () => {
    ...
    try {
      ...
      showToast({ type: 'success', title: 'Đã lưu dự án' })
    } catch (error) {
      showToast({
        type: 'error',
  ...
  const columns = [
    {
      key: 'name',
      header: 'Dự án',
      render: (item: ProjectSummary) => (
        <div>
          <strong>{item.name}</strong>
          <p className="text-xs">{item.investor}</p>
        </div>
      ),
    },
  ...
  return (
    <div>
      <div className="flex justify-between gap-4">
        <h2 className="text-3xl font-extrabold">Quản lý dự án</h2>
        <Button onClick={create}>Thêm dự án</Button>
      </div>
      ...
      >
        ...
        <Select
          label="Trạng thái"
          value={form.status}
          onChange={(e) => setForm({ ...form, status: e.target.value as ProjectRequest['status'] })}
        >
        ...
        <Input
          ...
          onChange={(e) => setForm({ ...form, priceFrom: Number(e.target.value) || undefined })}
        />
        <Input
          label="Giá đến"
          type="number"
          value={form.priceTo ?? ''}
          onChange={(e) => setForm({ ...form, priceTo: Number(e.target.value) || undefined })}
        />
      </EntityFormDialog>
      <DeleteConfirmation
